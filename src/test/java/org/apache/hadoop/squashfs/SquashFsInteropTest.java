...
public class SquashFsInteropTest {
	...
	public static Collection<Object[]> data() {
		return Arrays.asList(new Object[][] {
				...
				{ "file-with-cache", (ReaderCreator) (a -> createFileReaderWithCache(a)) },
				{ "mapped", (ReaderCreator) (a -> createMappedReader(a)) },
				{ "mapped-with-cache", (ReaderCreator) (a -> createMappedReaderWithCache(a)) } });
	}

	@FunctionalInterface
	public interface ReaderCreator {
		public SquashFsReader create(File archive) throws SquashFsException, IOException;
	}

	private ReaderCreator creator;

	public SquashFsInteropTest(String testName, ReaderCreator creator) {
		this.creator = creator;
	}

	public SquashFsReader createReader(File archive) throws SquashFsException, IOException {
		return creator.create(archive);
	}

	public static SquashFsReader createFileReader(File archive) throws SquashFsException, IOException {
		return SquashFsReader.fromFile(0, archive);
	}

	public static SquashFsReader createFileReaderWithCache(File archive) throws SquashFsException, IOException {
		MetadataBlockCache cache = new MetadataBlockCache(new TaggedMetadataBlockReader(true));
		return SquashFsReader.fromFile(0, archive, cache, new DataBlockCache(64), new DataBlockCache(64));
	}

	public static SquashFsReader createMappedReader(File archive) throws SquashFsException, IOException {
		MappedFile mmap;
	...
	public static SquashFsReader createMappedReaderWithCache(File archive) throws SquashFsException, IOException {
		MetadataBlockCache cache = new MetadataBlockCache(new TaggedMetadataBlockReader(false));
		MappedFile mmap;
		try (RandomAccessFile raf = new RandomAccessFile(archive, "r")) {
			mmap = MappedFile.mmap(raf.getChannel(),
					MappedSquashFsReader.PREFERRED_MAP_SIZE,
		...
		return SquashFsReader.fromMappedFile(0, mmap, cache, new DataBlockCache(64), new DataBlockCache(64));
	}
	...
	public void emptyArchiveShouldWork() throws Exception {
		...
		try (SquashFsReader reader = createReader(archive)) {
			assertEquals("wrong id count", (short) 1, reader.getSuperBlock().getIdCount());
			assertEquals("wrong inode count", 1, reader.getSuperBlock().getInodeCount());
			...
			assertSame("wrong inode type", INodeType.BASIC_DIRECTORY, root.getInodeType());
			assertEquals("wrong nlink count", 2, root.getNlink());
			assertEquals("wrong permissions", (short) 0700, root.getPermissions());
			assertEquals("wrong xattr index", INode.XATTR_NOT_PRESENT, root.getXattrIndex());
		}
	}
	...
	public void archiveWithDirectoriesAtRootShouldWork() throws Exception {
		...
		try (SquashFsReader reader = createReader(archive)) {
			assertEquals("wrong id count", (short) 1, reader.getSuperBlock().getIdCount());
			assertEquals("wrong inode count", 11, reader.getSuperBlock().getInodeCount());

			for (int i = 0; i < 10; i++) {
				INode dir = reader.findInodeByPath(String.format("/dir-%d", i));
				assertSame(String.format("wrong type for entry %d", i), INodeType.BASIC_DIRECTORY, dir.getInodeType());
			}
			...
			for (int i = 0; i < 10; i++) {
				...
				assertEquals(String.format("wrong name for entry %d", i), String.format("dir-%d", i),
	...
	public void archiveWithSyntheticDirectoryShouldWork() throws Exception {
		...
		try (SquashFsReader reader = createReader(archive)) {
			assertEquals("wrong id count", (short) 1, reader.getSuperBlock().getIdCount());
			assertEquals("wrong inode count", 2, reader.getSuperBlock().getInodeCount());
	...
	@Test
	public void archiveWithSyntheticDirectoryInReverseShouldWork() throws Exception {
		File archive = temp.newFile();
	...
	public void archiveWithFilesAtRootShouldWork() throws Exception {
			...
			for (int i = 0; i < 10; i++) {
				writer.entry(String.format("/file-%d", i))
						...
						.content(new ByteArrayInputStream(
								String.format("file-%d", i).getBytes(StandardCharsets.ISO_8859_1)))
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			for (int i = 0; i < 10; i++) {
				...
				assertSame(String.format("Wrong type for entry %d", i), INodeType.BASIC_FILE, file.getInodeType());
				try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
					reader.writeFileStream(file, bos);
					String content = new String(bos.toByteArray(), StandardCharsets.ISO_8859_1);
	...
	public void archiveWithFileContainingFullBlockShouldWork() throws Exception {
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			...
			assertSame("wrong inode type", INodeType.BASIC_FILE, entry.getInode().getInodeType());
			...
			assertEquals("wrong last modified", System.currentTimeMillis(), entry.getLastModified() * 1000L, 10000L);
			assertEquals("wrong data block count", 1, entry.getDataBlocks().size());
			assertNull("fragment found", entry.getFragment());
		}

		try (SquashFsReader reader = createReader(archive)) {
			assertEquals("wrong id count", (short) 3, reader.getSuperBlock().getIdCount());
			assertEquals("wrong inode count", 2, reader.getSuperBlock().getInodeCount());
			INode file = reader.findInodeByPath("/full.dat");
			assertSame("wrong type", INodeType.BASIC_FILE, file.getInodeType());
			assertEquals("wrong uid", 1000, reader.getIdTable().idFromIndex(file.getUidIdx()));
			assertEquals("wrong gid", 2000, reader.getIdTable().idFromIndex(file.getGidIdx()));
			...
			// test writing entire file
			{
				...
				INodeRef ref = reader.getExportTable().getInodeRef(file.getInodeNumber());
				INode file2 = reader.findInodeByInodeRef(ref);
				assertEquals("wrong inode number using write()", file.getInodeNumber(), file2.getInodeNumber());
			}
			...
			// test reading data
			{
				...
				try (ByteArrayOutputStream bos = new ByteArrayOutputStream()) {
					int c;
					long fileOffset = 0L;
					while ((c = reader.read(file, fileOffset, xfer, 0, xfer.length)) >= 0) {
						if (c >= 0) {
							bos.write(xfer, 0, c);
							fileOffset += c;
						}
				...
				INodeRef ref = reader.getExportTable().getInodeRef(file.getInodeNumber());
				INode file2 = reader.findInodeByInodeRef(ref);
				assertEquals("wrong inode number using read()", file.getInodeNumber(), file2.getInodeNumber());
			}
		}
	}

	@Test
	public void archiveWithFileContainingSparseBlockShouldWork() throws Exception {
		File archive = temp.newFile();
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertEquals("wrong sparse count", 131072L, ((FileINode) file).getSparse());
	...
	@Test
	public void archiveWithFileContainingAllSparseBlocksShouldWork() throws Exception {
		File archive = temp.newFile();
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertSame("wrong type", INodeType.EXTENDED_FILE, file.getInodeType());
			assertEquals("wrong sparse count", 262143L, ((FileINode) file).getSparse());
			assertEquals("wrong size", 262144L, ((FileINode) file).getFileSize());
	...
	public void archiveWithHardLinkToFileShouldWork() throws Exception {
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			...
			assertEquals("wrong target name", "/target.dat", source.getHardlinkTarget());
		...
		try (SquashFsReader reader = createReader(archive)) {
			assertEquals("wrong id count", (short) 2, reader.getSuperBlock().getIdCount());
			assertEquals("wrong inode count", 2, reader.getSuperBlock().getInodeCount());
			...
			assertSame("wrong inode number", file.getInodeNumber(), file2.getInodeNumber());
	...
	@Test
	public void archiveWithFileContainingMultipleBlocksAndFragmentShouldWork() throws Exception {
		File archive = temp.newFile();
	...
	public void archiveWithCharDeviceShouldWork() throws Exception {
		...
		try (SquashFsReader reader = createReader(archive)) {
			assertEquals("wrong id count", (short) 1, reader.getSuperBlock().getIdCount());
			assertEquals("wrong inode count", 3, reader.getSuperBlock().getInodeCount());

			INode dev = reader.findInodeByPath("/dev/zero");
			assertSame("wrong type", INodeType.BASIC_CHAR_DEVICE, dev.getInodeType());
			assertEquals("wrong device", (1 << 8) | 5, ((DeviceINode) dev).getDevice());
		}
	}
	...
	public void archiveWithBlockDeviceShouldWork() throws Exception {
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			assertSame("wrong type", INodeType.BASIC_BLOCK_DEVICE, dev.getInodeType());
	...
	public void archiveWithSymlinkShouldWork() throws Exception {
		...
		try (SquashFsReader reader = createReader(archive)) {
			...
			String target = new String(((SymlinkINode) link).getTargetPath(), StandardCharsets.ISO_8859_1);
	...
	public void fileWithNoTimestampShouldUseDefault() throws Exception {
		...
		try (SquashFsWriter writer = new SquashFsWriter(archive)) {
			...
			assertEquals("wrong timestamp", System.currentTimeMillis(), entry.lastModified * 1000L, 10000L);
		}
	}
