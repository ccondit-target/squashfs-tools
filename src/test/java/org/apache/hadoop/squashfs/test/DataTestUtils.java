...
public class DataTestUtils {
	...
	public static byte[] decompress(byte[] data) throws IOException {
		...
		try (ByteArrayInputStream bis = new ByteArrayInputStream(data)) {
			try (InflaterInputStream iis = new InflaterInputStream(bis, new Inflater(), 1024)) {
	...
	public static byte[] compress(byte[] data) throws IOException {
		...
		try (ByteArrayInputStream bis = new ByteArrayInputStream(data)) {
			try (ByteArrayOutputStream bos = new ByteArrayOutputStream(1024)) {
				try (DeflaterOutputStream dos = new DeflaterOutputStream(bos, new Deflater(), 1024)) {
					int c = 0;
	...
	public static byte[] saveFragmentMetadata(FragmentWriter fw) throws IOException {
	...
	public static byte[] decodeMetadataBlock(byte[] data) throws IOException {
		...
		try (MetadataBlockReader mbr = new MemoryMetadataBlockReader(tag, sb, data)) {
			...
			StringBuilder buf = new StringBuilder();
			BinUtils.dumpBin(buf, 17, "deserialized-data", output, 0, output.length, 16, 2);
			System.out.println(buf.toString());
