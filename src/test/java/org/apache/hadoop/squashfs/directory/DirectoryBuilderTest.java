...
public class DirectoryBuilderTest {
	...
	public void addShouldCreateEntry() {
		...
		assertEquals("wrong name", "test", new String(entry.name, StandardCharsets.ISO_8859_1));
	...
	public void addMultipleShouldCreateOnlyOneDirectoryHeader() {
		...
		assertSame("wrong class for entry 0", DirectoryHeader.class, db.elements.get(0).getClass());
		assertSame("wrong class for entry 1", DirectoryEntry.class, db.elements.get(1).getClass());
		assertSame("wrong class for entry 2", DirectoryEntry.class, db.elements.get(2).getClass());
	}
	...
	public void addMultipleShouldCreateMultipleDirectoryHeadersIfStartBlockChanges() {
		...
		assertSame("wrong class for entry 0", DirectoryHeader.class, db.elements.get(0).getClass());
		assertSame("wrong class for entry 1", DirectoryEntry.class, db.elements.get(1).getClass());
		assertSame("wrong class for entry 2", DirectoryHeader.class, db.elements.get(2).getClass());
		assertSame("wrong class for entry 3", DirectoryEntry.class, db.elements.get(3).getClass());
	}
	...
	public void addMultipleShouldCreateOnlyOneDirectoryHeaderIf256Entries() {
		...
		assertSame("wrong class for entry 0", DirectoryHeader.class, db.elements.get(0).getClass());
		for (int i = 1; i <= 256; i++) {
			assertSame("wrong class for entry " + i, DirectoryEntry.class, db.elements.get(i).getClass());
		}
	}
	...
	public void addMultipleShouldCreateMultipleDirectoryHeadersIf257Entries() {
		...
		assertSame("wrong class for entry 257", DirectoryHeader.class, db.elements.get(257).getClass());
		assertSame("wrong class for entry 258", DirectoryEntry.class, db.elements.get(258).getClass());
	}
	...
	public void writeShouldSerializeData() throws Exception {
		...
		List<DirectoryElement> elements = DirectoryTestUtils.deserializeDirectory(data);
		assertEquals("wrong length", 2, elements.size());
		assertEquals("wrong type for element 0", DirectoryHeader.class, elements.get(0).getClass());
		assertEquals("wrong type for element 1", DirectoryEntry.class, elements.get(1).getClass());
		...
		assertEquals("wrong name", "test", new String(entry.getName(), StandardCharsets.ISO_8859_1));
	}
