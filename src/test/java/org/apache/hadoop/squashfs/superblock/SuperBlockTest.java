...
public class SuperBlockTest {
	...
	public void modificationTimePropertyShouldWorkAsExpected() {
		assertEquals((double) System.currentTimeMillis(), (double) (sb.getModificationTime() * 1000L), 5000d);
	...
	public void flagsPropertyShouldWorkAsExpected() {
		assertEquals(SuperBlockFlag.flagsFor(SuperBlockFlag.EXPORTABLE, SuperBlockFlag.DUPLICATES), sb.getFlags());
	...
	public void hasFlagShouldCorrectlyDetectFlags() {
		assertTrue("Missing exportable flag", sb.hasFlag(SuperBlockFlag.EXPORTABLE));
		assertTrue("Missing duplicates flag", sb.hasFlag(SuperBlockFlag.DUPLICATES));
		assertFalse("Has always fragments flag", sb.hasFlag(SuperBlockFlag.ALWAYS_FRAGMENTS));
	}
	...
	public void writeDataAndReadDataShouldBeReflexive() throws Exception {
		...
		assertEquals("Wrong modification time", sb.getModificationTime(), sb2.getModificationTime());
		assertEquals("Wrong block size", sb.getBlockSize(), sb2.getBlockSize());
		assertEquals("Wrong fragment entry count", sb.getFragmentEntryCount(), sb2.getFragmentEntryCount());
		assertSame("Wrong compression ID", sb.getCompressionId(), sb2.getCompressionId());
		...
		assertEquals("Wrong root inode ref", sb.getRootInodeRef(), sb2.getRootInodeRef());
		assertEquals("Wrong bytes used", sb.getBytesUsed(), sb2.getBytesUsed());
		assertEquals("Wrong id table start", sb.getIdTableStart(), sb2.getIdTableStart());
		assertEquals("Wrong xattr id table start", sb.getXattrIdTableStart(), sb2.getXattrIdTableStart());
		assertEquals("Wrong inode table start", sb.getInodeTableStart(), sb2.getInodeTableStart());
		assertEquals("Wrong directory table start", sb.getDirectoryTableStart(), sb2.getDirectoryTableStart());
		assertEquals("Wrong fragment table start", sb.getFragmentTableStart(), sb2.getFragmentTableStart());
		assertEquals("Wrong export table start", sb.getExportTableStart(), sb2.getExportTableStart());
	}
